...
def extract_features(record,featureEnd):
    featureSeason=[convert_float(field)  for  field in record[0]] 
    features=[convert_float(field)  for  field in record[1: featureEnd]]
...
def PredictData(sc,model): 
    ...
    df =sc.parallelize([[str(process_id),str(l_value),str(a_value),str(b_value) ,str(h) ,str(s),str(v),str(take_time),str(water)]])
    #print(type(df))
    #print(df.first())
    #a = rawData.sparkContext.parallelize(rdd.collect())
    #print(type(rawData))
    #rawData=[str(process_id)+","+str(l_value)+","+str(a_value)+","+str(b_value)+","+str(h)+","+str(s)+","+str(v)+","+str(take_time)+","+str(water)]
    #lines = rawData.map(lambda x: x.split(","))
    #lines=[str(process_id),str(l_value),str(a_value),str(b_value),str(h),str(s),str(v),str(take_time),str(water)]
    #print (type(lines))
    #print (lines.first())
    #print("共計：" + str(lines.count()) + "筆")
    #----------------------2.建立訓練評估所需資料 LabeledPoint RDD-------------
    labelpointRDD = df.map(lambda r: LabeledPoint(extract_label(r),extract_features(r,len(r) - 1)))
    #----------------------3.定義字典----------------
    ...
    #----------------------4.進行預測並顯示結果--------------
    #print(type(labelpointRDD))
    for lp in labelpointRDD.take(1):
...
#                            "  , 實際:" + str(label) + result +",  誤差:" + str(error)
        dataDesc="  特徵: 乾燥製程id "+str(features[0]) +", L_value "+\
                            str(features[1])+ ", a_value " +\
                            str(features[2]) +  ",b_value"+ \
                            str(features[3]) +","+\
                            "h "+str(features[4])+", s "+ \
                           str(features[5]) +\
                           "v "+str(features[6])+", take_time "+ \
                           str(features[7]) +\
                            " ==> 預測結果:" + str(predict )+\
                            "  , 實際:" + str(label) + result +",  誤差:" + str(error)
